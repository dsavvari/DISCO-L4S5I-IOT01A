                      ...
                      iotc_timed_task_handle_t timed_task, void *user_data)
{
    ...
    if (message_received) {
		...
		printf("%d messages left, closing down\n", max_try_receive);
		...
		const char messageFormat[] = "{ \"temperature\": %f, \"humidity\": %f, \"pressure\": %f }";
		char messageData[256] = {0};
		sprintf(messageData, messageFormat, BSP_TSENSOR_ReadTemp(), BSP_HSENSOR_ReadHumidity(), BSP_PSENSOR_ReadPressure());

		printf("%d messages left to send, or until we receive a reply\n", max_try_receive);
		printf("publishing msg \"%s\"\n", messageData);

		iotc_state_t state = iotc_publish(context_handle, 
				MBED_APP_MQTT_TOPIC, 
				messageData, 
...
int main(int argc, char *argv[])
{
    ...
    printf("**** Starting Initialise Sensors ****\n");
    initialiseSensors();
	...
	auto wait = true;
	while (wait) {
		auto status = net->get_connection_status();
		printf("Network status %d \n", status);
		if ((NSAPI_STATUS_LOCAL_UP == status) || (NSAPI_STATUS_GLOBAL_UP==status)) {
			wait = false;
		}
